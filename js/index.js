...
const cent = new THREE.Vector3(0, 0, -camera.position.z);
cent.applyQuaternion(camera.quaternion);
cent.add(camera.position);
  ...
  if (zoom < 5) {
    if(flag!=true){
      ...
      // Create a new instance of OrbitControls that is tied to the mesh object
      

      // Set the mesh position to the center of the scene
      

      // Set the position of the mesh to the center point
      mesh.position.copy(cent);
  ...
  // If the zoom level is above the threshold and the mesh is currently shown, hide it
    ...
    scene.remove(mesh);
     model1.position.copy(cent).add(new THREE.Vector3(0, -0.7, 0));
    scene.add(model1);
